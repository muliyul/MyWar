...
public class War {
	...
	private static War loadMenu(IOHandler io) {
		...
		int choice = -1;
		do {
			choice = io.getChoice("\nWar menu", new String[]{"Enter manually",
			"Parse from XML"});
	...
	private int showWarMenu(IOHandler io, String warName) {
		return io.getChoice("\nHere comes the menu for "+ warName +" !", new String[]{"Add missile",
				"Add launcher", "Add Iron-Dome", "Add artillery",
				"Add target to destroy", "Show inventory" ,isActive ? "End war and show stats"
						: "Start war"});
	}
	...
	private void showInventory(IOHandler io) {
		...
		for (IronDome id : domes) {
			io.showMessege(id + (id.getTargets().size() > 0 ? ": targets"
					+ id.getTargets().toString() : ""));
		}
		io.showMessege("Artillery:");
		for (Artillery a : artillery) {
			io.showMessege(a +  (a.getTargets().size() > 0 ? ": targets"
		...
		if(selection < 1 || selection > 2){
			...
			if(selection==1){
				...
				if(availableMissiles.size() == 0){
					io.showMessege("There are no missiles to target right now");
					return true;
				}
				else {
				...
				i=0;
				if(launchers.size() == 0){
					io.showMessege("There are no launchers to target right now");
					return true;
				}
				else {
					...
					selectedLauncher = launchers.get(selection + selectionOffset);
